...
use OpenPGP\MCrypt\Wrapper;
use OpenPGP\Message\Message;
use OpenPGP\OpenPGP;
use OpenPGP\Packet\AsymmetricSessionKeyPacket;
use OpenPGP\Packet\EncryptedDataPacket\IntegrityProtectedDataPacket;
use OpenPGP\Packet\ModificationDetectionCodePacket;
use OpenPGP\Packet\PublicKeyPacket\PublicKeyPacket;
use OpenPGP\Packet\SymmetricSessionKeyPacket;
use OpenPGP\S2k;
...
class Symmetric {
    public static function encrypt($passphrases_and_keys, $message, $symmetric_algorithm=9) {
        ...
        $mdc = new ModificationDetectionCodePacket(hash('sha1', $to_encrypt . "\xD3\x14", true));
        $to_encrypt .= $mdc->to_bytes();
        $encrypted = array(new IntegrityProtectedDataPacket($cipher->encrypt($to_encrypt)));
        ...
        foreach($passphrases_and_keys as $pass) {
            if($pass instanceof PublicKeyPacket) {
                ...
                array_unshift($encrypted, new AsymmetricSessionKeyPacket($pass->algorithm, $pass->fingerprint(), $esk));
            } else if(is_string($pass)) {
                $s2k = new S2k(Random::string(10));
                $cipher->setKey($s2k->make_key($pass, $key_bytes));
                $esk = $cipher->encrypt(chr($symmetric_algorithm) . $key);
                array_unshift($encrypted, new SymmetricSessionKeyPacket($s2k, $esk, $symmetric_algorithm));
            }
        }
    ...
    public static function decryptSymmetric($pass, $m) {
        ...
        foreach($m as $p) {
            if($p instanceof SymmetricSessionKeyPacket) {
    ...
    public static function decryptPacket($epacket, $symmetric_algorithm, $key) {
        ...
        if($epacket instanceof IntegrityProtectedDataPacket) {
            ...
            case 3:
                if(defined('MCRYPT_CAST_128')) {
                    if(function_exists('mcrypt_encrypt') && defined('MCRYPT_MODE_CFB')) {
                        $cipher = new Wrapper(MCRYPT_CAST_128);
                    }
                } else {
    ...
    public static function getEncryptedData($m) {
        foreach($m as $p) {
            if($p instanceof \OpenPGP\Packet\EncryptedDataPacket\EncryptedDataPacket) return $p;
        }
